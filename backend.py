...
class Api:
    ...
    def _read_custom_tools(self):
        return self._read_preferences().get("custom_tools", [])

    def get_custom_tools(self):
        return self._read_custom_tools()

    def browse_custom_tool_folder(self):
        try:
            import webview
    ...
    def add_custom_tool(self, name, path, skills_subfolder=None):
        try:
            name = (name or "").strip()
            if not name:
                raise ValueError("Enter a name for this tool")
            if len(name) > 60:
            ...
            destination = Path(self.backup_dir).resolve()
            if resolved == destination or resolved.is_relative_to(destination) or destination.is_relative_to(resolved):
                raise ValueError("This folder overlaps with the backup destination")
            for root in self._engine.roots().values():
                try:
                    if resolved == Path(root).resolve():
                        raise ValueError("This is already tracked as a known tool's folder")
                except OSError:
                    continue
            tools = self._read_custom_tools()
            slug = re.sub(r"[^a-z0-9]+", "-", name.lower()).strip("-") or "tool"
            base_id = "custom_" + slug
            tool_id = base_id
    ...
    def remove_custom_tool(self, tool_id):
        try:
            tools = [t for t in self._read_custom_tools() if t["id"] != tool_id]
            self._update_preferences(custom_tools=tools)
            return {"success": True, "tools": tools}
        except Exception as error:
            return {"success": False, "error": str(error)}
